...
- Verify the fix with curl or by asking the user to check in their browser
- Ask the user for a before/after screenshot if the fix is visual

```bash
curl -s -o /dev/null -w "HTTP %{http_code}" <affected-url>
```
