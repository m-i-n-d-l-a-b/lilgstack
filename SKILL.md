...
version: 1.2.0
description: |
  Garry's Stack (gstack) — Claude Code skills toolkit. Routes to specialized skills
  for QA, code review, design, shipping, benchmarking, and more. Use when you want
  skill-based assistance or when you invoke any /skill command. (gstack)
allowed-tools:
  - Bash
  - Read
  - AskUserQuestion
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"inspiring-montalcini","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.gstack/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.claude/skills/gstack/bin/gstack-timeline-log '{"skill":"inspiring-montalcini","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
